...
def sample_cfg_properties(args, device, generative_model, dataset_info, target_properties, property_norms, guidance_scale=2.0, n_samples=100, fix_noise=False):
    max_n_nodes = dataset_info['max_n_nodes']
    
    # Sample nodes per molecule (simplification: max nodes or from distribution)
    # For now, let's just use 19 nodes (typical for QM9) if not provided
    nodesxsample = torch.tensor([19] * n_samples)
    ...
    # Build context from target properties
    context_list = []
    for prop_key in args.conditioning:
        raw_value = target_properties[prop_key]
        mean = property_norms[prop_key]['mean']
        mad = property_norms[prop_key]['mad']
        normalized = (raw_value - mean) / mad
        context_list.append(normalized)

    # context: [n_samples, max_n_nodes, n_properties]
    context_vector = torch.tensor(context_list, device=device, dtype=torch.float32)
    context = context_vector.unsqueeze(0).unsqueeze(1)  # [1, 1, n_props]
    context = context.expand(n_samples, max_n_nodes, -1)  # broadcast
    context = context * node_mask
    
    x, h = generative_model.sample_cfg(
        n_samples=n_samples,
        n_nodes=max_n_nodes,
        node_mask=node_mask,
        edge_mask=edge_mask,
