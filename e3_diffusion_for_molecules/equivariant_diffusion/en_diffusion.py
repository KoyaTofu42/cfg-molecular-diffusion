...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def sample_p_zs_given_zt(self, s, t, zt, node_mask, edge_mask, context, fix_noise=False, eps_t_override=None):
        ...
        # Neural net prediction (or use override for CFG).
        if eps_t_override is not None:
            eps_t = eps_t_override
        else:
    ...
    @torch.no_grad()
    def sample_cfg(self, n_samples, n_nodes, node_mask, edge_mask, context,
                   guidance_scale=2.0, fix_noise=False):
        """
        Draw samples using Classifier-Free Guidance (CFG).
        Performs two forward passes per timestep: conditional and unconditional.
        """
        if fix_noise:
            z = self.sample_combined_position_feature_noise(1, n_nodes, node_mask)
        else:
        ...
        # Create null context (zeros) for unconditional pass.
        ...
        for s in reversed(range(0, self.T)):
            ...
            eps_cond = self.phi(z, t_array, node_mask, edge_mask, context)
            eps_uncond = self.phi(z, t_array, node_mask, edge_mask, null_context)

            # Guided combination: eps = eps_uncond + w * (eps_cond - eps_uncond)
            eps_guided = eps_uncond + guidance_scale * (eps_cond - eps_uncond)
            # === END CFG ===

            z = self.sample_p_zs_given_zt(
                s_array, t_array, z, node_mask, edge_mask, context,
                fix_noise=fix_noise, eps_t_override=eps_guided)
