...
def train_epoch(args, loader, epoch, model, model_dp, model_ema, ema, device, dtype, property_norms, optim,
    ...
    for i, data in enumerate(loader):
        ...
        if len(args.conditioning) > 0:
            ...
            # === CFG NULL MASKING ===
            if hasattr(args, 'cfg_dropout_prob') and args.cfg_dropout_prob > 0:
                batch_size = x.size(0)
                cfg_dropout_mode = getattr(args, 'cfg_dropout_mode', 'joint')
                if cfg_dropout_mode == 'joint':
                    # All-or-nothing: mask entire condition vector
                    mask = (torch.rand(batch_size, 1, 1, device=device) 
                            < args.cfg_dropout_prob).float()
                    context = context * (1 - mask)  # Zero out masked samples
                elif cfg_dropout_mode == 'independent':
                    # Independent: mask each property separately
                    n_props = context.size(2)
                    mask = (torch.rand(batch_size, 1, n_props, device=device) 
                            < args.cfg_dropout_prob).float()
                    context = context * (1 - mask)
            # === END CFG NULL MASKING ===
